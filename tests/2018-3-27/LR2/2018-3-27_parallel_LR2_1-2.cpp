    ...
    for (int len = min_; len <= max_; len += step_)
    {
        ...
        for (int j = 0; j < inside_; j++)
        {
            ...
            if (rank == 0)
            {
                ...
                for (i = 0; i < size; i++)
                {
                    MPI_Isend(sendbuf, len, MPI_CHAR, i, 0, MPI_COMM_WORLD, &requests[i]);
                }
                MPI_Waitall(size, requests, MPI_STATUS_IGNORE);
            }
            else
            {
                MPI_Irecv(recvbuf, len, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &request);
        ...
        for (int j = 0; j < inside_; j++)
        {
            // ----------------- Mpi
            start = MPI_Wtime();
            MPI_Bcast(sendbuf, len, MPI_CHAR, 0, MPI_COMM_WORLD);
            stop = MPI_Wtime();
            local_time += stop - start;
            // ----------------- Mpi
        }
