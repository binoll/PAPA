/*
 *   3. Смоделировать последовательный двусторонний обмен сообщениями между двумя процессами,
 *      замерить с помощью функции MPI_Wtime() время на одну итерацию обмена,
 *      определить зависимость времени от длины сообщения.
 */
...
int main(int argc, char *argv[])
{
    ...
    if (rank == 0) {
        MPI_Send(sendbuf, strlen(sendbuf), MPI_CHAR, 1, tag0, MPI_COMM_WORLD);
        ...
        MPI_Send(sendbuf, strlen(sendbuf), MPI_CHAR, 0, tag1, MPI_COMM_WORLD);
    }
    time_stop = MPI_Wtime();
