...
from controllers.detectors import fingerprint
...
class PAPA:
    ...
    def __init__(self, es, tokenzierFunction, tokenFileContent):
        self.es = es
        self.tokenizer = tokenzierFunction
        self.tokensFileContent = tokenFileContent
    ...
    def add(self, filename, fileContent):
        """
            Добавление в базу
        """

        text = fileContent if type(fileContent) == list else fileContent.split('\n')
    ...
    def PAPA(self, fileContent, fileName, source):
        """
            Проверка кода с базой
        """

        
        tokens = self.tokenizer(fileContent, self.tokensFileContent)
        tokenstring = ''.join([x[0] for x in tokens])
        fingerprints = fingerprint.fingerprints(tokens, int(os.environ.get("K")), int(os.environ.get("T")))
