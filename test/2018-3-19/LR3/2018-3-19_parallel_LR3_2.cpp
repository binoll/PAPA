/*
Pазработать параллельную программу, демонстрирующую
использование производных типов данных совместно с 
операциями коллективного взаимодействия на примере 
квадратной матрицы произвольного размера. Размер 
квадратной матрицы должен задаваться в качестве 
входного параметра. Элементы матрицы должны 
...
int main(int argc, char* argv[])
{
    ...
    // объявляем свой тип данных
    MPI_Datatype ROW;
    // конструктор типов смежных данных
    MPI_Type_contiguous(N, MPI_INT, &ROW);
    // закрепление типа данных перед информационным обменом
    MPI_Type_commit(&ROW);
    ...
    // задаем свой тип данных
    MPI_Datatype column, xpose;
    // определение диапазона от первого байта до последнего
    MPI_Type_extent(MPI_INT, &sizeofint);
    // векторный конструктор
    MPI_Type_vector(N, 1, N, MPI_INT, &column);
    // модифицированный векторный конструктор
