...
int main(int argc, char* argv[])
{
    ...
    int opt, pr = 1;
    int **Matrix_A, **Matrix_A_transposed;
    ...
    Matrix_A_transposed= (int**)malloc(size*sizeof(int*));
    ...
    for (i = 0; i < size; i++) {
        ...
        Matrix_A_transposed[i] = (int*)malloc(size * sizeof(int));
        #pragma omp parallel for schedule(dynamic, 1) private(j)
        for (j = 0; j < size; j++) {
            Matrix_A[i][j] = rand() % 10;
            Matrix_A_transposed[i][j] = 0;
        }
    }
    ...
    for (i = 0; i < size; i++) {
        #pragma omp parallel for schedule(dynamic, 1) private(j)
        for (j = 0; j < size; j++) {
            Matrix_A_transposed[j][i] = Matrix_A[i][j];
        }
    }
    end = omp_get_wtime();
    ...
    if (pr == 1) {
        ...
        print_matrix(size, size, Matrix_A_transposed);
    }

    // Сhecking the correctness of transposition
    for (i = 0; i < size; i++) {
        for (j = 0; j < size; j++) {
            if (Matrix_A_transposed[i][j] != Matrix_A[j][i]) {
                printf("Error transposition on position (%d %d)\n", i, j);
                exit(EXIT_FAILURE);
            }
        }
    }
    ...
    //#pragma omp parallel for schedule(dynamic, 1) private(j)
        ...
        free((void *)Matrix_A_transposed[i]);
    }
    free(Matrix_A);
    free(Matrix_A_transposed);
