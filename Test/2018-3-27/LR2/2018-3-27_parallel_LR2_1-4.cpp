...
int main(int argc, char** argv)
{
    ...
    {
        // Все выделяют память для приема
        recvbuf = (char*)malloc(len * sizeof(char));
        ...
        for (int j = 0; j < inside_; j++)
        {
            ...
            start = MPI_Wtime();
            // --------------- Scatter
            MPI_Irecv(recvbuf, len, MPI_CHAR, 0, 0, MPI_COMM_WORLD, &request);
            ...
            if (rank == 0) {
                ...
                for (i = 0; i < size; i++) {
                    MPI_Isend(&sendbuf[i * len], len, MPI_CHAR, i, 0, MPI_COMM_WORLD, &requests[i]);
                }
        ...
        for (int j = 0; j < inside_; j++)
        {
            // ----------------- Mpi
            start = MPI_Wtime();
            MPI_Scatter(sendbuf, len, MPI_CHAR, recvbuf, len, MPI_CHAR, 0, MPI_COMM_WORLD);
            stop = MPI_Wtime();
            local_time += stop - start;
            // ----------------- Mpi
        }
