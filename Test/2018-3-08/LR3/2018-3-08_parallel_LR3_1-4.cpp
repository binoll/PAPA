...
int main(int argc, char* argv[])
{
    ...
    // ******* Library function MPI_Scan with user operation ******* //
    recvbuf = (int*)malloc(message_slice * sizeof(int));
    MPI_Scan(sendbuf, recvbuf, message_slice, MPI_INT, operation, MPI_COMM_WORLD);
    if (!rank) {
        printf("# Result of the operation MPI_Scan with reduce operation - <%s> \n", operation_message);
    }
    // To correctly display the result on the screen by printf()
    MPI_Barrier(MPI_COMM_WORLD);
    // Output only in the mode of particular message transmission
